...
/**
 * @Description: 开始节点actor
 * @Author: xuqianlei
 * @Date: 2024/01/02/13:48
 */
@Slf4j
public class StartNodeActor extends AbstractBehavior<CommandContext> implements FsmBehavior<StartNode> {
     ...
     * 当前节点
     */
    private StartNode currNode;


    public static Behavior<CommandContext> create(ActorServiceFactory actorServiceFactory) {
        return Behaviors.setup(context -> Behaviors.withTimers(timers -> new StartNodeActor(context, actorServiceFactory, timers)));
    }

    public StartNodeActor(ActorContext<CommandContext> context, ActorServiceFactory actorServiceFactory, TimerScheduler<CommandContext> timerScheduler) {
        super(context);
    ...
    public Receive<CommandContext> createReceive() {
        return newReceiveBuilder()
                ...
                .onSignal(PostStop.class, signal -> {
                    log.info("StartNode节点已停止 " + getContext().getSelf().path());
                    return this;
                })
                .build();
    }
    ...
    public Behavior<CommandContext> onInit(NodeCommandContext command) {
        NodeCommandContext<StartNode> commandContext = (NodeCommandContext<StartNode>) command;
